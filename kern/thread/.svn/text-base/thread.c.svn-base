...
thread_bootstrap(void)
{
  ...
  struct pid_list* temp = kmalloc(sizeof(struct pid_list));
  if (temp == NULL) {
    kfree(me);
    panic("thread_bootstrap: Out of memory\n");
  }
	    ...
	    struct thread **ret)
{
  ...
  //kprintf("return thread_fork success\n");
	return 0;
 ...
 fail:
  ...
  kprintf("return thread_fork fail\n");
	return result;
}
...
thread_exit(void)
{
    ...
    lock_acquire(pid_list_lock);
    struct pid_list* curr = curthread->ptr;
    struct pid_list* parent = get_process(curr->ppid);
