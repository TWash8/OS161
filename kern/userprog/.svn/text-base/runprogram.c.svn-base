...
runprogram(char *progname,char** argv,unsigned long argc)
{
	...
	//kprintf("am i even here ???\n");


	int argvptr[argc],bufferlen,i;
	//int o;
	int totaloffset=0;
	for(i = argc-1; i >= 0; i--) {
		//kprintf("am i in teh loop ???\n");
		...
		//kprintf("argv is %s ???\n",argv[i]);
		//kprintf("argc is %d ???\n",argc);
		copyoutstr(argv[i], (userptr_t)stackptr, (size_t)length, &bufferlen);
		//kprintf("%d\n",argvptr[i]);
		argvptr[i] = stackptr;
	}


	argvptr[argc] = (int)NULL;
	for(i = argc; i >=0; i--)
	{
		stackptr = stackptr - 4;
