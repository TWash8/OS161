...
{
	//kprintf("LOAD ELF IS GETTING CALLED -----------------------------------------------------------------------------\n");
  ...
  int heapstart;
    ...
    if (counter == 0) {
      ...
      as->fileSize1 = ph.p_filesz;
      as->memSize1 = ph.p_memsz;
    } else if (counter == 1) {
      as->offset2 = ph.p_offset;
      as->fileSize2 = ph.p_filesz;
      as->memSize2 = ph.p_memsz;
    } else {
      panic("wtf? Counter is greater than 2?");
    }
#endif

		as->progheader[counter]=ph; 
    size_t size = (ph.p_memsz + (ph.p_vaddr & (~(vaddr_t)PAGE_FRAME)) + PAGE_SIZE - 1) & PAGE_FRAME;

    if (counter == 0) {
      as->as_npages1=size>>12;
      as->as_vbase1=(ph.p_vaddr&PAGE_FRAME);
    } else if (counter == 1) {
      as->as_npages2=size>>12;
      as->as_vbase2=(ph.p_vaddr&PAGE_FRAME);
    } else {
      panic("Shouldn't be here.\n");
    }

    if (heapstart < (ph.p_vaddr & PAGE_FRAME) + size) {
      heapstart = (ph.p_vaddr & PAGE_FRAME) + size;
    }
